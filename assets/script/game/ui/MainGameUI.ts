...
export class MainGameUI extends CCComp {
    ...
    private showBetPanel(): void {
        ...
        // 在动画期间禁用所有按钮交互
        this.betAmountSelector.setBetItemsInteractable(false);
        ...
        tween(this.betPanel)
            ...
            .call(() => {
                ...
                this.betAmountSelector.setBetItemsInteractable(true);
    ...
    onDestroy() {
        ...
        // 清理下注面板中的按钮事件由BetAmountSelector组件自己处理
                ...
                if (autoCashOutUI) {
                    autoCashOutUI.onOpen(params,
                        (multiplier: number, totalBets: number) => {
                            ...
                            // 开始自动提现回调
                            betting.lastCashOutMultiplier = multiplier;
    ...
    private onGuideEvent(event:string,data:any){
        if(event == "GUIDE_SHOW_BETPANEL"){
            this.node.off(Node.EventType.TOUCH_END, this.onGlobalTouch, this);
            this.showBetPanel();
            this.betAmountSelector.snapToItem(6);
