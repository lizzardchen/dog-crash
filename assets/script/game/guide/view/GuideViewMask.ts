 ...
 */
import { Component, EventTouch, instantiate, Node, Prefab, UITransform, v2, Widget, _decorator, PolygonCollider2D } from "cc";
...
export class GuideViewMask extends Component {
    ...
    start() {
        ...
        const polygon2d = this.mask.getComponent(PolygonCollider2D);
        if(!polygon2d){
            this.mask.addComponent(PolygonCollider2D);
        }
    ...
    /** 绘制遮罩 */
    draw(btn: Node) {
        let guideViewItem = btn.getComponent(GuideViewItem);
        if (guideViewItem) {
            this._guideTouchType = guideViewItem.guideTouchType;
        } else {
